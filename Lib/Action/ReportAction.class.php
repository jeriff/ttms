...
class ReportAction extends Action {
    ...
    public function user_task()
	{
		...
		$week_arr = D("simplifyTask")->check_time($task_frequency,$date_time);
	...
	public function task_export(){
		...
		if($_GET){
			...
			/*导出指定用户的任务，默认是全部*/
			if($_GET['created_by'] == '' || $_GET['created_by'] == 'quanbu')//执行人
			{
				$created_by = 'quanbu';
				$this->assign('created_by', $created_by);
			}else{
				$created_by = $_GET['created_by'];
				$where['created_by'] = array('like','%'.$created_by.'%');
				$this->assign('created_by', $created_by);
			}
			/*时间转换，转换为周的日期时间段*/
			$week_arr = $simplifytask->check_time("week",$date_arr[0]);
			/*任务导出，以任务为主键导出*/
			if($_GET['export_type'] == 1){
				$field = array('created_by','task_name','task_type','task_status','begin_time','end_time','remark');
				$where['_string'] = " (begin_time >= '".$week_arr[0]."' and begin_time<= '".$week_arr[1]."') or (end_time >= '".$week_arr[0]."' and end_time<= '".$week_arr[1]."')";
				$task_list = $simplifytask->where($where)->order('created_by')->field($field)->select();
				/*获取导出字段的列名*/
		...
		$listparent = $operator_arr->get_optname($mark = '┣', $parentid = 0,$created_by);
		$listparent = '<select name="created_by" id="created_by"><option value="quanbu">全部</option>' . $listparent . '</select>';
